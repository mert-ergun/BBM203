...
ImageMatrix ImageMatrix::operator+(const ImageMatrix &other) const {
    ...
    // Create a new ImageMatrix object to store the result
    ImageMatrix result(height, width);
    
    // Iterate over the elements of both matrices and add them together
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
...
ImageMatrix ImageMatrix::operator-(const ImageMatrix &other) const {
    ...
    // Iterate over the elements of both matrices and subtract them from each other
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; j++) {
            result.data[i][j] = data[i][j] - other.data[i][j];
        }
...
ImageMatrix ImageMatrix::operator*(const double &scalar) const {
    ...
    // Iterate over the elements of the matrix and multiply them with the scalar
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; j++) {
